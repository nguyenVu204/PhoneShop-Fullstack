...
export default function InvoicePage() {
  ...
  useEffect(() => {
    const fetchOrderDetail = async () => {
      try {
        // --- SỬA LẠI: Gọi trực tiếp API chi tiết đơn hàng ---
        const res = await axiosClient.get(`/orders/${id}`);
        
        // Giả sử API trả về object đơn hàng ngay trong res.data
        if (res.data) {
            setOrder(res.data);
        } else {
  ...
  if (!order) return (
    <div className="h-screen flex flex-col items-center justify-center bg-gray-100 gap-4">
        <div className="text-red-500 font-bold text-lg">Đơn hàng không tồn tại hoặc đã bị xóa.</div>
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-8 flex flex-col items-center font-sans">
      ...
      <div className="w-full max-w-[210mm] mb-6 flex justify-between items-center md:flex-row flex-col gap-4 no-print">
        <Link to="/admin/orders" className="flex items-center text-gray-600 hover:text-blue-600 transition font-medium">
          <ArrowLeft size={20} className="mr-2"/> Quay lại danh sách
        </Link>
        <button 
          onClick={handlePrint}
          disabled={isPrinting}
          className="bg-blue-600 text-white px-6 py-2.5 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Printer size={20}/>
      ...
      <div id="invoice-content" className="bg-white w-[210mm] min-h-[297mm] p-12 shadow-2xl text-slate-800 relative">
        ...
        <div className="flex justify-between items-start border-b-2 border-slate-800 pb-8 mb-8">
            ...
            <div className="text-right">
                ...
                {/* Trạng thái thanh toán (Giả định) */}
                <div className="mt-4">
        ...
        <div className="grid grid-cols-2 gap-12 mb-10">
            <div>
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    {/* Nếu có email khách hàng thì hiển thị thêm ở đây */}
                </div>
            </div>
            <div>
                    ...
                    <tr key={item.id}>
                        <td className="p-4">
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">
                                Phân loại: {item.productVariant?.color} - {item.productVariant?.rom}
                            </p>
                        </td>
                        <td className="p-4 text-right font-medium text-sm text-slate-600">
                            {item.unitPrice.toLocaleString('vi-VN')} ₫
                        </td>
                        <td className="p-4 text-center font-medium text-sm text-slate-600">
                            {item.quantity}
                        </td>
                        <td className="p-4 text-right font-bold text-sm text-slate-800">
        ...
        <div className="flex justify-end mb-16">
            <div className="w-1/2 space-y-3">
                <div className="flex justify-between text-sm text-gray-600">
                    <span>Tạm tính:</span>
                    <span className="font-medium">{order.totalAmount.toLocaleString('vi-VN')} ₫</span>
                </div>
                <div className="flex justify-between text-sm text-gray-600">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl font-black text-blue-600">{order.totalAmount.toLocaleString('vi-VN')} ₫</span>
                </div>
